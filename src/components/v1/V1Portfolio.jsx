...
import ProjectPage from '../SEO/ProjectPage';
import SEOHead from '../SEO/SEOHead';
import { PersonSchema, WebSiteSchema, BreadcrumbSchema, ProjectSchemas } from '../SEO/JSONLD';
import '../../index.css';

export default function V1Portfolio({ projects, fetchProjects, isLoggedIn, setIsLoggedIn }) {
  const [activeFilter, setActiveFilter] = useState('All');
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [showCMS, setShowCMS] = useState(false);
  const [selectedProject, setSelectedProject] = useState(null);
  const [projectPageView, setProjectPageView] = useState(null);
    ...
    const checkRoute = () => {
      ...
      if (hash === '#/cms') {
        ...
        const project = projects.find(p => {
          const pSlug = (p.name || p._id).toLowerCase().replace(/[^\w]+/g, '-').replace(/^-|-$/g, '');
          return pSlug === slug;
        });
        if (project) {
          setProjectPageView(project);
        }
      } else {
        setShowCMS(false);
        setProjectPageView(null);
      }
    };
    checkRoute();
    window.addEventListener('hashchange', checkRoute);
    return () => window.removeEventListener('hashchange', checkRoute);
  }, [projects, isLoggedIn, setIsLoggedIn]);

  useEffect(() => {
    return () => {
      if (sendTickTimerRef.current) clearTimeout(sendTickTimerRef.current);
      if (contactMessageFadeTimerRef.current) clearTimeout(contactMessageFadeTimerRef.current);
      if (contactMessageClearTimerRef.current) clearTimeout(contactMessageClearTimerRef.current);
    };
  }, []);
  ...
  if (projectPageView) {
    return <ProjectPage project={projectPageView} onClose={handleProjectPageClose} />;
  }
  ...
  const breadcrumbItems = [
    { name: 'Home', url: 'https://ashwinchoudhury.me' },
  ];
  ...
  return (
    <>
      ...
      <header>
        ...
        >
          <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <a href="#intro" className="text-xl font-bold bg-gradient-to-r from-[#76B2F0] to-[#F61BA9] bg-clip-text text-transparent no-underline" aria-label="Home">
              Ashwin
            </a>

            <div className="hidden md:flex gap-2" role="menubar">
              <a href="#intro" role="menuitem" className="px-4 py-1 rounded text-sm relative group overflow-hidden no-underline">
              ...
              <a href="#projects" role="menuitem" className="px-4 py-1 rounded text-sm relative group overflow-hidden no-underline">
                <span className="absolute inset-0 rounded border-2 border-gray-600 group-hover:border-transparent transition"></span>
                <span className="absolute inset-0 rounded opacity-0 group-hover:opacity-100 transition">
                  <span className="absolute inset-[-100%] bg-[conic-gradient(from_0deg,#76B2F0,#F61BA9,#76B2F0,#F61BA9,#76B2F0)] gradient-border-animate"></span>
                </span>
              ...
              <a href="#skills" role="menuitem" className="px-4 py-1 rounded text-sm relative group overflow-hidden no-underline">
                <span className="absolute inset-0 rounded border-2 border-gray-600 group-hover:border-transparent transition"></span>
                <span className="absolute inset-0 rounded opacity-0 group-hover:opacity-100 transition">
                  <span className="absolute inset-[-100%] bg-[conic-gradient(from_0deg,#76B2F0,#F61BA9,#76B2F0,#F61BA9,#76B2F0)] gradient-border-animate"></span>
                </span>
              ...
              <a href="#contact" role="menuitem" className="px-4 py-1 rounded text-sm relative group overflow-hidden no-underline">
                <span className="absolute inset-0 rounded border-2 border-gray-600 group-hover:border-transparent transition"></span>
                <span className="absolute inset-0 rounded opacity-0 group-hover:opacity-100 transition">
                  <span className="absolute inset-[-100%] bg-[conic-gradient(from_0deg,#76B2F0,#F61BA9,#76B2F0,#F61BA9,#76B2F0)] gradient-border-animate"></span>
                </span>
      ...
      <main className="min-h-screen bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] text-white" style={{ marginTop: 0, paddingTop: 0 }}>
        <section id="intro" className="border-b border-gray-800 flex items-center justify-center" style={{ paddingTop: '80px', minHeight: 'calc(100vh - 80px)' }} aria-label="Introduction">
          <div className="max-w-7xl mx-auto px-6 py-16">
            <div className="flex flex-col md:flex-row items-center justify-center gap-12">
              <div className="relative flex-shrink-0">
                <div className="w-64 h-64 rounded-full bg-gradient-to-br from-[#76B2F0] to-[#F61BA9] p-1">
                  <div className="w-full h-full rounded-full bg-[#171717] overflow-hidden">
                    <img
                      src="/profile.png"
                      alt="Ashwin Choudhury - Full-Stack Web Developer"
                      loading="eager"
                      decoding="async"
              ...
              <div className="max-w-xl text-center md:text-left">
                <h1 className="text-4xl md:text-5xl font-bold mb-2">Ashwin Choudhury</h1>
        ...
        <section id="projects" className="border-b border-gray-800" aria-label="Projects">
          <div className="max-w-7xl mx-auto px-6 py-12">
            <div className="mb-8">
              <h2 className="text-2xl font-bold mb-6">My Projects</h2>

              <div className="flex gap-3 mb-8 flex-wrap" role="group" aria-label="Filter projects by category">
                {filters.map((filter) => (
                  <button
                    key={filter}
                  ...
                  return (
                    ...
                    >
                      <a
                        ...
                        className="block w-full h-full no-underline text-white"
                          ...
                          <div className="flex gap-2">
                            {project.githubLink && (
                              <a
                                ...
                                className="text-xs px-3 py-1.5 rounded font-semibold relative group/btn overflow-hidden flex-1 text-center no-underline"
                              >
        ...
        {selectedProject && !projectPageView && (
          <div
            ...
            >
              ...
              <div className="relative z-10 bg-[#0f0f0f]">
                {selectedProject.image && (
                  <figure className="relative h-56 md:h-64">
                    <img
                ...
                <div className="p-8">
                  ...
                  <h2 className="text-4xl md:text-5xl font-mono tracking-tight mb-4">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {selectedProject.demoLink && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-5 py-2 rounded font-semibold relative group overflow-hidden no-underline"
                      >
                        <span className="absolute inset-0 rounded opacity-100 group-hover:opacity-0 transition">
                          <span className="absolute inset-0 rounded bg-gradient-to-r from-[#76B2F0] to-[#F61BA9] p-[2px]">
                            <span className="absolute inset-[2px] rounded bg-[#0f0f0f]"></span>
        ...
        <section id="skills" className="border-b border-gray-800" aria-label="Skills">
          <div className="max-w-7xl mx-auto px-6 py-12">
            <div className="mb-8">
        ...
        <section id="contact" className="border-b border-gray-800" aria-label="Contact">
          <div className="max-w-7xl mx-auto px-6 py-16">
